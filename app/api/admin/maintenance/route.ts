...
import { clearDeviceAlerts, clearRequestLogs } from "@/lib/admin-db";

// 危险操作统一入口：
//   { action: "clear-logs" }                    清空全部请求日志（含错误详情）
//   { action: "clear-logs", olderThanDays: 30 } 清理 30 天前的请求日志
//   { action: "clear-alerts" }                  清空设备告警
export async function POST(request: NextRequest) {
  ...
  try {
    switch (body.action) {
      ...
      case "clear-alerts": {
        const deleted = await clearDeviceAlerts();
        console.log(`admin cleared ${deleted} device alerts`);
        return NextResponse.json({ ok: true, deleted });
      }
      default:
